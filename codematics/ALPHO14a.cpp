...
long long int getSumUtil(long long int *st,
...
long long int *constructST(int arr[], int n)
{
    ...
    int max_size = 2*(int)pow(2.0, (double)x) - 1; //Maximum size of segment tree
//    long long int *st = new (long longint[max_size];
    long long int *st = (long long int*)malloc(max_size*(sizeof(long long)));
...
int main() {
    ...
    while(t--) {
        ...
        while(m--) {
            ...
            if(type == 1) {
                cout<<getSum(st, n, first, second)<<'\n';
            }
            else {
                updateValue(arr, st, n, first, arr[first]+second);
                updateValue(arr, st, n, first+1, arr[first+1]-second);
            }    
        }    
